...
{
    public int $max_geodes = 0;    // keep score of the max geodes
    ...
    /* 0=time 1=ore 2=clay 3=obsidian 4=geode 5=ore_robot 6=clay_robot 7=obsidian_robot 8=geode_robot */
    public function mine(array $state) : array
    {
        $state[0]--;                    // subtract a minute
        $state[1] += $state[5];         // mine ore
        $state[2] += $state[6];         // mine clay
        $state[3] += $state[7];         // mine obsidian
        $state[4] += $state[8];         // mine geodes
        return $state;
    }
    ...
    {
        /* not enough ore */
        if ($state[1] < $bp->geode_robot_ore) return false;

        /* not enough obsidian */
        if ($state[3] < $bp->geode_robot_obsidian) return false;
    ...
    {
        $state[8]++;                                // geode robot
        $state[1] -= $bp->geode_robot_ore;          // ore
        $state[3] -= $bp->geode_robot_obsidian;     // obsidian
        return $state;
    }
    ...
    {
        /* not enough ore */
        if($state[1] < $bp->obsidian_robot_ore) return false;

        /* not enough clay */
        if($state[2] < $bp->obsidian_robot_clay) return false;

        /* dont build more obsidian robots than the max cost of the geode robot */
        if ($state[7] >= $bp->max_obsidian_cost) return false;
    ...
    {
        $state[7]++;                                // obsidian robot
        $state[1] -= $bp->obsidian_robot_ore;       // ore
        $state[2] -= $bp->obsidian_robot_clay;      // clay
        return $state;
    }
    ...
    {
        /* not enough ore */
        if ($state[1] < $bp->clay_robot_ore) return false;

        /* never build more clay robots than the max cost of the obsidian robot */
        if ($state[6] >= $bp->max_clay_cost) return false;
    ...
    {
        /* not enough ore */
        if ($state[1] < $bp->ore_robot_ore) return false;

        /* never build more ore robots than the largest ore cost */
        if ($state[5] >= $bp->max_ore_cost) return false;
